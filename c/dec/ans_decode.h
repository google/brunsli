...
struct ANSDecodingData {
  ...
  ANSSymbolInfo map_[BRUSNLI_ANS_TAB_SIZE];
};
...
class ANSDecoder {
 public:
  ...
  int ReadSymbol(const ANSDecodingData& code, BrunsliInput* in) {
    const uint32_t res = state_ & (BRUSNLI_ANS_TAB_SIZE - 1);
    const ANSSymbolInfo& s = code.map_[res];
    state_ = s.freq_ * (state_ >> BRUSNLI_ANS_LOG_TAB_SIZE) + s.offset_;
    if (state_ < (1u << 16u)) {
      state_ = (state_ << 16u) | in->GetNextWord();
    }
    return s.symbol_;
  }
  bool CheckCRC() const { return state_ == (BRUSNLI_ANS_SIGNATURE << 16u); }
