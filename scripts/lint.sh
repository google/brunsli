...
# TODO(eustas): integrate fix-headers and other clang-tidy checks
# parallel -j ${nprocs} --keep-order -- clang-tidy -config-file=scripts/clang-tidy-check-headers.cfg -p build/ -format-style=file -quiet "$@" {} \
#   < <(git ls-files | grep -E '(\.cc|\.cpp|\.h)$') \
